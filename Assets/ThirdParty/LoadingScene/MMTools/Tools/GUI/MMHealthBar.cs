...
namespace MoreMountains.Tools
{
    ...
    public class MMHealthBar : MonoBehaviour 
	{
		...
		//-----------------    My Custom           ---------------------------------------------------
		[Header("Health Customization")]
		[SerializeField] private Gradient _healthForegroundMid;
		[SerializeField] private Gradient _healthForegroundLow;
		...
		protected virtual void UpdateDrawnColors()
		{
			...
			{
				if (_progressBar.BarProgress > 0.6f)
				{
					_foregroundImage.color = ForegroundColor.Evaluate(_progressBar.BarProgress);
				}
				else if(_progressBar.BarProgress <= 0.6f && _progressBar.BarProgress > 0.3f)
				{
					_foregroundImage.color = _healthForegroundMid.Evaluate(_progressBar.BarProgress);
				}
				else
				{
					_foregroundImage.color = _healthForegroundLow.Evaluate(_progressBar.BarProgress);
				}
